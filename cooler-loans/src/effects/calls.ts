 ...
 * `.foo()` semantics: a reverted (or empty-data) call resolves to `null`.
 *
 ...
 * call read state that changes over time; immutable metadata (decimals,
 * tokens, factory) is left unpinned.
 *
 ...
 * JSON-serialized into the COOLER_CALL_MOCK env var (worker threads copy the
...
export const REVERTED = Symbol.for("cooler-loans.reverted");

/**
 * Declarative call-mock spec for offline tests. `value` is a JSON-friendly
 * representation; tuples are encoded as `kind: "tuple"` with nested results.
 */
...
export type CallMockRule = {
  /** function name to match (e.g. "interestReceivables") */
  fn: string;
...
export const CALL_MOCK_ENV = "COOLER_CALL_MOCK";
 ...
 *
 * In mock mode the *decoded* shape is returned directly (mocks define the
 * decoded value, including tuples as arrays), bypassing viem encode/decode.
 */

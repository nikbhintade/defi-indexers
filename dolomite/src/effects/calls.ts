 ...
 * so handler logic can mirror the original `reverted` branches.
 *
 * All calls go through a single `ethCall` Effect (cache: true) carrying raw
 * calldata, so every distinct (to, data, block) tuple is cached. Calls that
 * read state which changes over time (getMarketPrice, balanceOf) are
 * block-pinned to event.block.number; immutable ERC20 metadata
 * (name/symbol/decimals) and admin risk-param getters are left unpinned.
 *
 * For offline tests, a mock is installed via `setCallMock`, serialized into
 * the DOLOMITE_CALL_MOCK env var (envio runs handlers in a worker thread that
...
export const REVERTED = Symbol.for("dolomite.reverted");
...
export const CALL_MOCK_ENV = "DOLOMITE_CALL_MOCK";
 ...
 * undecodable output. Pass `context.effect` inside onEvent handlers (cached
 * Effect); pass `null` outside handler contexts (raw call). `block` pins the
 * call to a block (state-dependent reads). `mockFn` overrides the function
 * name used for mock matching.
 */

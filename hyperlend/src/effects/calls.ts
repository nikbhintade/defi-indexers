/**
 * eth_call layer. Mirrors the Ponder helpers in `src/helpers/getPrice.ts`,
 * which do `context.client.readContract(...)` (pinned to the event block).
 *
 * All calls flow through a single cached `ethCall` Effect carrying raw
 * calldata, so every distinct (to, data, block) tuple is cached and re-runs are
 * deterministic. Price/state reads are block-pinned.
 *
 ...
 * HYPERLEND_CALL_MOCK env var (worker threads copy the parent env at creation).
 *
 * Adapted from the aave-v3 reference port.
 */
...
export const REVERTED = Symbol.for("hyperlend.reverted");
...
export const CALL_MOCK_ENV = "HYPERLEND_CALL_MOCK";
...
function getClient(): PublicClient {
  if (!client) {
    client = createPublicClient({ transport: http(process.env.RPC_URL_999, { batch: true }) });
  }
  return client;
}
 ...
 * - `mockFn`: override the function name used for mock matching.
 */

...
import { loadPublicAllocatorVault } from "./publicAllocator";
...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    // MetaMorpho.load (non-critical): skip if the vault was not indexed.
    const mm = await context.MetaMorpho.get(low(event.srcAddress));
    if (!mm) return;
    context.MetaMorpho.set({ ...mm, name: event.params.name });
  },
);
...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    const mm = await context.MetaMorpho.get(low(event.srcAddress));
    if (!mm) return;
    context.MetaMorpho.set({ ...mm, symbol: event.params.symbol });
  },
);
...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    ...
    const rate = await cloneRate(context, mm.rate_id, ev.block.timestamp);
...
indexer.onEvent(
  ...
  async ({ event, context }) => {
    ...
    const isPublicAllocator =
      getPublicAllocatorAddress() === low(event.params.allocator);
    ...
    if (isPublicAllocator && event.params.isAllocator) {
      mm = { ...mm, hasPublicAllocator: true };
      // The public allocator vault can be created before the allocator is set,
      // so we link the allocator here.
      const publicAllocatorVault = await loadPublicAllocatorVault(
        context,
        low(event.srcAddress),
      );
      context.MetaMorphoPublicAllocator.set({
        ...publicAllocatorVault,
        allocator_id: allocator.id,
      });
    } else {
      mm = { ...mm, hasPublicAllocator: false };
    }
    context.MetaMorpho.set(mm);

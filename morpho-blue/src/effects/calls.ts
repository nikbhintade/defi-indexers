 ...
 * JSON-serialized into the MORPHO_BLUE_CALL_MOCK env var (worker threads copy the
...
export const REVERTED = Symbol.for("morpho-blue.reverted");

export type MockResult =
  | { kind: "bigint"; value: string }
  | { kind: "number"; value: number }
  | { kind: "string"; value: string }
  // For tuple-returning calls (e.g. latestRoundData) values are positional.
  | { kind: "tuple"; value: Array<string | number> }
  | { kind: "revert" };

export type CallMockRule = {
  /** function name to match (e.g. "decimals") */
  fn: string;
...
export const CALL_MOCK_ENV = "MORPHO_BLUE_CALL_MOCK";
 ...
 *   pass `null` outside handler contexts (raw call).

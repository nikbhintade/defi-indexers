 ...
 * symbol, name, coins, base_pool, underlying) is left unpinned.
 *
 * `contractRegister` handlers have no `context.effect`, so they use the raw
 * (uncached) path through the same code.
 *
 * For offline tests, a mock can be installed via `setCallMock` (stored on
 * globalThis so module-instance identity doesn't matter).
 */
...
export const REVERTED = Symbol.for("curve-volume.reverted");
...
export type CallMockRule = {
  /** function name to match (e.g. "coins") */
  fn: string;
...
export const CALL_MOCK_ENV = "CURVE_VOLUME_CALL_MOCK";

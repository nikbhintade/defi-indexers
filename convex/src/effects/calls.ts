 ...
 * call read state that changes over time (virtualPrice, balances, totalSupply,
 * rewardRate, get_dy, latestAnswer); immutable metadata (decimals, symbol,
 * name, coins, minter, underlying, tokenInfo) is left unpinned.
 *
 * `contractRegister` handlers have no `context.effect`, so they use the raw
 * (uncached) path through the same code (pass `null` as the effect caller).
 *
 * For offline tests, a mock can be installed via `setCallMock` (stored in an
 * env var so the worker thread that runs handlers inherits it by value).
 */
...
export const REVERTED = Symbol.for("convex.reverted");

/**
 * Declarative call-mock spec for offline tests. The envio test indexer runs
 * handlers in a worker thread, so the mock must be passed by value: it's
 * JSON-serialized into an env var (worker threads copy the parent env at
 * creation time).
 */
...
export type CallMockRule = {
  /** function name to match (e.g. "poolInfo") */
  fn: string;
...
export const CALL_MOCK_ENV = "CONVEX_CALL_MOCK";
...
function materializeMock(result: MockResult): unknown {
  switch (result.kind) {
    case "revert":
      return REVERTED;
    case "bigint":
    ...
    case "bool":
      return result.value;
    case "tuple":
      return result.value.map(materializeMock);
  }
}
...
function resolveMock(spec: CallMockSpec, req: CallRequest): unknown {
  for (const rule of spec.rules) {
    ...
    return materializeMock(rule.result);
  }
  if (spec.strict) {
 ...
 *   Effect); pass `null` in contractRegister handlers (raw call).

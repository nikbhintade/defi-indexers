 ...
 * are deterministic. The calls this port performs are:
 *  - ERC20 metadata reads (name / symbol / decimals + their bytes32 fallbacks,
 *    totalSupply) — immutable, therefore unpinned.
 *  - Pair.balanceOf(account) for LP positions — state-dependent, block-pinned.
 *
 * The original's other eth_call (`Factory.getPair` in pricing) is NOT performed
 * here: QuickSwap's findEthPerToken iterates `token.whitelist` (a per-token list
 * of pair addresses written on PairCreated), not a factory lookup.
 *
 * For offline tests, a mock can be installed via `setCallMock`. The envio test
 * indexer runs handlers in a worker thread, so the mock is passed by value:
 * JSON-serialized into the QUICKSWAP_CALL_MOCK env var (worker threads copy the
 * parent env at creation time).
 */
...
export const REVERTED = Symbol.for("quickswap.reverted");
...
export type CallMockRule = {
  /** mock function name to match (e.g. "symbol", "symbolBytes32", "decimals", "totalSupply", "balanceOf") */
  fn: string;
...
export const CALL_MOCK_ENV = "QUICKSWAP_CALL_MOCK";
...
function getClient(): PublicClient {
  if (!client) {
    client = createPublicClient({
      transport: http(process.env.RPC_URL_137, { batch: true }),
    });
  }
  return client;
}

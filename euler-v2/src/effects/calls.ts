/**
 * eth_call layer. Mirrors the subgraph's `Contract.bind(addr).foo()` /
 * `try_foo()` semantics: a reverted (or empty-data) call resolves to `null`.
 *
 * The source's tracking.ts (src/utils/tracking.ts) binds the EulerVault at the
 * event address and calls:
 *   - balanceOf(account)        — non-try (would throw/revert the handler)
 *   - try_debtOf(account)       — try-call; reverted => debt stays 0
 * Both reads are state-dependent (live share balance / debt) so they are
 ...
 * JSON-serialized into the EULER_V2_CALL_MOCK env var (worker threads copy the
...
export const REVERTED = Symbol.for("euler-v2.reverted");
...
export type CallMockRule = {
  /** function name to match (e.g. "balanceOf") */
  fn: string;
...
export const CALL_MOCK_ENV = "EULER_V2_CALL_MOCK";
 ...
 *   Effect). `null` would mean a raw call (only used outside handlers).

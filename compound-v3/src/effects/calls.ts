 ...
 * semantics: a reverted (or empty-data / undecodable) call resolves to
 * `null`. graph-node also treats output-decoding failures as reverts, which
 * the rewardConfig V2-vs-V1 fallback relies on.
 *
 ...
 * are deterministic. Calls are block-pinned whenever the original subgraph
 * call read state that changes over time (all Comet / Configurator /
 * CometRewards / Chainlink reads); immutable ERC20 metadata (name, symbol,
 * decimals) is left unpinned.
 *
 ...
 * value: JSON-serialized into the COMPOUND_V3_CALL_MOCK env var (worker
 * threads copy the parent env at creation time).
 */
...
export const REVERTED = Symbol.for("compound-v3.reverted");

/**
 * Declarative call-mock spec for offline tests.
 * - "json" carries tuple/array results; bigints encoded as decimal strings
 *   (wrappers convert with BigInt()).
 */
...
export type CallMockRule = {
  /** function name to match (e.g. "totalsBasic", "rewardConfigV2", "receiptTopics") */
  fn: string;
  /** optional contract address (or tx hash for receiptTopics) to match (case-insensitive) */
  to?: string;
  /** optional args to match; compared as lowercase strings */
  args?: string[];
  /** optional block to match (block-pinned reads carry the event block) */
  block?: number;
  result: MockResult;
};
...
export const CALL_MOCK_ENV = "COMPOUND_V3_CALL_MOCK";
...
/**
 * Receipt-log topic0 list for a transaction; the original subgraph used
 * `event.receipt.logs` (receipt: true) in handleTransfer to detect
 * Supply/Withdraw/AbsorbDebt events in the same transaction. envio has no
 * receipt-log field_selection, so this is fetched (and cached) via RPC.
 */
export const receiptTopics = createEffect(
  {
    name: "receiptTopics",
    input: { hash: S.string },
  ...
  async ({ input }) => {
    try {
      const receipt = await getClient().getTransactionReceipt({ hash: input.hash as `0x${string}` });
      return receipt.logs.map((l) => (l.topics.length > 0 ? (l.topics[0] as string) : "0x"));
    } catch (_e) {
      return null;
    }
  },
...
/**
 * Receipt topic0 list with `try_` semantics (null when receipt unavailable).
 * Mockable via rule `{ fn: "receiptTopics", to: <txHash>, result: { kind:
 * "json", value: [topics...] } }`.
 */
export async function tryGetReceiptTopics(
  effectCall: EffectCaller | null,
  txHash: string,
): Promise<string[] | null> {
  const mockSpec = getCallMockSpec();
  if (mockSpec) {
    const result = resolveMock(mockSpec, { to: txHash.toLowerCase(), fn: "receiptTopics", args: [] });
    return result === REVERTED ? null : (result as string[]);
  }
  if (effectCall) {
    return effectCall(receiptTopics, { hash: txHash });
  }
  try {
    const receipt = await getClient().getTransactionReceipt({ hash: txHash as `0x${string}` });
    return receipt.logs.map((l) => (l.topics.length > 0 ? (l.topics[0] as string) : "0x"));
  } catch (_e) {
    return null;
  }
 ...
 * - `mockFn`: override the function name used for mock matching (used to
 *   distinguish rewardConfig V1 vs V2, which share calldata).
 */

 ...
 * are deterministic. The only calls this port performs are ERC20 metadata
 * reads (name / symbol / decimals + their bytes32 fallbacks), which are
 * immutable and therefore unpinned. The original's other eth_call
 * (`Factory.getPair` in findBnbPerToken) is replaced by the PairTokenLookup
 * entity (see src/pricing.ts and MIGRATION.md).
 *
 ...
 * value: JSON-serialized into the PANCAKE_EXCHANGE_CALL_MOCK env var (worker
...
export const REVERTED = Symbol.for("pancakeswap-exchange.reverted");
...
export type CallMockRule = {
  /** mock function name to match (e.g. "symbol", "symbolBytes32", "decimals") */
  fn: string;
...
export const CALL_MOCK_ENV = "PANCAKE_EXCHANGE_CALL_MOCK";

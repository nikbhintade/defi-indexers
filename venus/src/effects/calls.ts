 ...
 * non-try `.foo()` semantics: a reverted (or empty-data) call resolves to
 * `null`, which callers map to the subgraph's revert sentinels
 * (NOT_AVAILABLE_BIG_INT = -1, nullAddress for addresses, etc.).
 *
 ...
 * are deterministic. State-dependent reads are block-pinned; immutable
 * metadata (decimals, symbol, name, underlying, comptroller) is unpinned.
 *
 ...
 * JSON-serialized into the VENUS_CALL_MOCK env var (worker threads copy the
...
export const REVERTED = Symbol.for("venus.reverted");
...
export type MockResult =
  ...
  /** tuple of bigints, e.g. venusBorrowState -> (index, block) */
  | { kind: "bigintTuple"; value: string[] }
  | { kind: "revert" };

export type CallMockRule = {
  /** function name to match (e.g. "exchangeRateStored") */
  fn: string;
...
export const CALL_MOCK_ENV = "VENUS_CALL_MOCK";
  ...
  for (const rule of spec.rules) {
    ...
    switch (result.kind) {
      ...
      case "string":
        return result.value;
      case "bigintTuple":
        return result.value.map((v) => BigInt(v));
    }
  }
  if (spec.strict) {
    throw new Error(

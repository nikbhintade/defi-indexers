/**
 * eth_call layer. Mirrors the Ponder services' `readContractSafe(...)` /
 * viem `readContract` calls: a reverted (or empty-data) call resolves to `null`.
 *
 ...
 * are deterministic. Calls are block-pinned whenever the original read was
 * state-dependent (balances, totalSupply); immutable metadata (decimals,
 * symbol, name) is left unpinned.
 *
 ...
 * value: JSON-serialized into the ANEMOY_CALL_MOCK env var.
 */
...
export const REVERTED = Symbol.for("anemoy-centrifuge.reverted");
...
export type CallMockRule = {
  /** function name to match (e.g. "totalSupply") */
  fn: string;
...
export const CALL_MOCK_ENV = "ANEMOY_CALL_MOCK";
...
/**
 * `readContractSafe` / `try_<fn>` equivalent. Returns decoded result or null on revert.
...
// ---- typed convenience wrappers for the reads this port performs ----

export const erc20TotalSupply = (effectCall: EffectCaller | null, to: string, block?: number) =>

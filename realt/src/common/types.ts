 ...
 *
 * Original AssemblyScript helpers take `event: ethereum.Event`; the port passes
 * the envio event object typed structurally. graph-node entities are mutable;
 * envio entities are readonly, so helpers mutate `{ ...stored }` copies and
 * write back with `context.X.set`.
 */

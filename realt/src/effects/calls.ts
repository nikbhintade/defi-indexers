 ...
 * `.try_foo()` semantics: a reverted / empty / undecodable call resolves to
 * `null`.
 *
 * Adapted from the aave-v3 reference port. All calls flow through a single
 * cached `ethCall` Effect carrying raw calldata, so every distinct (to, data,
 * block) tuple is cached and re-runs are deterministic. State reads are
 ...
 * REALT_CALL_MOCK env var (worker threads copy the parent env at creation).
 */
...
export const REVERTED = Symbol.for("realt.reverted");
...
export const CALL_MOCK_ENV = "REALT_CALL_MOCK";
...
function getClient(): PublicClient {
  if (!client) {
    client = createPublicClient({
      transport: http(process.env[`RPC_URL_${CHAIN_ID}`], { batch: true }),
    });
  }
  return client;
}
